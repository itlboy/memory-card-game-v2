...
// Đủ cho cấp cuối (bàn 8×11 = 44 cặp) — bộ 30 cũ chết ngay ở bàn 8×9.
...
describe('bàn của mọi cấp đều đầy và cân', () => {
  ...
  /**
   * Trần 88 thẻ (8×11). Hai cỡ cuối — 72 và 88 thẻ — CỐ Ý phá ngưỡng chạm
   ...
   */
  it('trần 88 thẻ, và chỉ hai cỡ cuối được phá ngưỡng 44px', () => {
    const sizes = [...new Set(boards.map((b) => b.spec.pairs * 2))].sort((a, b) => a - b);
    expect(Math.max(...sizes)).toBe(88);
    expect(sizes.filter((n) => n > 56)).toEqual([72, 88]);
  });
});
