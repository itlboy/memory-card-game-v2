...
var QUICK_EMOJIS = ["\u{1F44D}", "\u{1F602}", "\u{1F62E}", "\u{1F62D}", "\u{1F525}", "\u{1F389}", "\u{1F914}", "\u{1F4A9}"];
    ...
    switch (msg.t) {
      ...
      case "again": {
        ...
        if (this.room.players.length < ROOM_LIMITS.minPlayers) {
          this.send(ws, { t: "error", code: "not-enough", message: "C\u1EA7n \xEDt nh\u1EA5t 2 ng\u01B0\u1EDDi ch\u01A1i" });
          return;
        }
        this.startGame();
        await this.save();
  ...
  roomInfo() {
    ...
    return {
      ...
      players: room.players.map((p) => {
        ...
        return gp ? publicPlayer(gp, this.connected(p.id)) : {
          id: p.id,
          name: p.name,
          avatar: p.avatar,
          score: 0,
